...
//     echo $this->getKey();
  ...
  function getUrlLink() {
    if ($this->hasField('url')) return ' <a'.(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'').' href="'.$this->getField('url').'"><img src="images/icone_pdf.gif" alt="[pdf]" /></a>';
    return '';
  }
  ...
  */
  function toTR() {
        ...
        if (BIBTEXBROWSER_BIBTEX_LINKS) {
          ...
          echo " <a".(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'')." class=\"biburl\" title=\"".$this->getKey()."\" {$href}><img src=\"images/icone_bib.gif\" alt=\"[bibtex]\" /></a>";
        }
        ...
        if ($this->hasField('doi')) {
            echo ' <a href="http://dx.doi.org/'.$this->getField("doi").'"><img src="images/icone_doi.jpg" alt="[doi]" /></a>';
        }
...
class AcademicDisplay  {
  ...
  function search2html($query, $title) {
    ...
    if (count($entries)>0) {
    echo "\n".'<div class="btb-header" style="margin-top:20px;padding-left:30px;margin-right:10px;font-family:constanz, comic;font-weight:bold;font-size:20pt;border-bottom-style:solid;border-bottom-color:#000000;border-bottom-width:2px;">'.$title.'</div>'."\n";
...
if (method_exists($content, 'getCSS')) {
  echo $content->getCSS(); 
} else if (is_readable('layout/Ibisc.css')) {
  readfile('layout/Ibisc.css');
}
